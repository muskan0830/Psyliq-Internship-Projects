...
--ANS: 1. Identify and eliminate redundancy by normalizing the database. Break down the data into smaller, related tables to avoid storing the same information in multiple places.
...
--3. Use the patient_id as a primary key in this table and reference it as a foreign key in other relevant tables.

--Q19: Explain how you can optimize the performance of SQL queries on this dataset.
--ANS: 1. Identify columns used in WHERE clauses, JOIN conditions, and ORDER BY clauses and create indexes on those columns. This can significantly speed up data retrieval.
--2. Only select the columns that are necessary for your query. Avoid using SELECT * if you don't need all columns, as fetching unnecessary data can impact performance.